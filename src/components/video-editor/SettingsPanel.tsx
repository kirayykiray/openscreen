...
import type { ZoomDepth, CropRegion, CornerSettings } from "./types";
...
interface SettingsPanelProps {
  ...
  onCornerSettingsChange?: (settings: CornerSettings) => void;
  cornerRadius?: number;
...
export function SettingsPanel({ selected, onWallpaperChange, selectedZoomDepth, onZoomDepthChange, selectedZoomId, onZoomDelete, shadowIntensity = 0, onShadowChange, showBlur, onBlurChange, cropRegion, onCropChange, cornerSettings, onCornerSettingsChange, cornerRadius = 0, onCornerRadiusChange, videoElement, onExport, showCursor = true, onShowCursorChange, hasCursorData = false, cursorSettings = DEFAULT_CURSOR_SETTINGS, onCursorSettingsChange, padding = 20, onPaddingChange }: SettingsPanelProps) {
    ...
    <div className="flex-[2] min-w-0 bg-[#09090b] border border-white/5 rounded-2xl p-4 flex flex-col shadow-xl h-full overflow-y-auto custom-scrollbar">
      ...
      <div className="mb-6">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-xl bg-white/5 border border-white/5 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-xs font-medium text-slate-200">Corners</div>
              <span className="text-[10px] text-slate-400 font-mono">{cornerSettings?.radius ?? cornerRadius}px</span>
            </div>
            <Slider
              value={[cornerSettings?.radius ?? cornerRadius]}
              onValueChange={(values) => {
                if (onCornerSettingsChange && cornerSettings) {
                  onCornerSettingsChange({ ...cornerSettings, radius: values[0] });
                } else {
                  onCornerRadiusChange?.(values[0]);
                }
              }}
              min={0}
              max={100}
            ...
            <div className="flex gap-1 mt-2">
              {CORNER_PRESETS.map((preset) => {
                const currentRadius = cornerSettings?.radius ?? cornerRadius;
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (onCornerSettingsChange && cornerSettings) {
                        onCornerSettingsChange({ ...cornerSettings, radius: preset.value });
                      } else {
                        onCornerRadiusChange?.(preset.value);
                      }
                    }}
                    className={cn(
                      "flex-1 py-1 px-1 text-[9px] font-medium rounded transition-all",
                      isActive
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
            <div className="text-xs font-medium text-slate-200">Blur BG</div>
            <Switch
              checked={showBlur}
          ...
          {hasCursorData && (
            <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs font-medium text-slate-200">Cursor</div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setShowCursorSettings(!showCursorSettings)}
                  className="text-[10px] text-[#34B27B] hover:text-[#34B27B]/80"
                >
                  Settings
                </button>
                <Switch
          ...
          {hasCursorData && showCursorSettings && onCursorSettingsChange && (
            <div className="col-span-2 p-3 rounded-xl bg-white/5 border border-white/5 space-y-3">
              <div className="text-xs font-medium text-slate-200 mb-2">Cursor Appearance</div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-1">
                  {(['small', 'medium', 'large', 'xlarge'] as const).map((size) => (
                    <button
                      key={size}
                      onClick={() => onCursorSettingsChange({ ...cursorSettings, size })}
                      className={cn(
                        "flex-1 py-1.5 text-[10px] font-medium rounded transition-all capitalize",
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                  <span className="text-[10px] text-slate-300">Glow</span>
                  <Switch
                    checked={cursorSettings.showGlow}
                    onCheckedChange={(checked) => onCursorSettingsChange({ ...cursorSettings, showGlow: checked })}
                    className="data-[state=checked]:bg-[#34B27B] scale-75"
                  />
                </div>
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                  <span className="text-[10px] text-slate-300">Click Ripple</span>
                  <Switch
                    checked={cursorSettings.showRipple}
                    onCheckedChange={(checked) => onCursorSettingsChange({ ...cursorSettings, showRipple: checked })}
                    className="data-[state=checked]:bg-[#34B27B] scale-75"
                  />
                </div>
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                  <span className="text-[10px] text-slate-300">Highlight</span>
                  <Switch
                    checked={cursorSettings.showHighlight}
                    onCheckedChange={(checked) => onCursorSettingsChange({ ...cursorSettings, showHighlight: checked })}
                    className="data-[state=checked]:bg-[#34B27B] scale-75"
                  />
                </div>
                <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                  <span className="text-[10px] text-slate-300">Motion Blur</span>
                  <Switch
                    checked={cursorSettings.motionBlur}
                    onCheckedChange={(checked) => onCursorSettingsChange({ ...cursorSettings, motionBlur: checked })}
                    className="data-[state=checked]:bg-[#34B27B] scale-75"
                  />
                </div>
              </div>
              ...
              {/* Glow Intensity */}
              {cursorSettings.showGlow && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-400">Glow Intensity</span>
                    <span className="text-[10px] text-slate-500 font-mono">{Math.round(cursorSettings.glowIntensity * 100)}%</span>
                  </div>
                  <Slider
                    value={[cursorSettings.glowIntensity]}
                    onValueChange={([value]) => onCursorSettingsChange({ ...cursorSettings, glowIntensity: value })}
                    min={0}
                    max={1}
                    step={0.1}
                    className="[&_[role=slider]]:bg-[#34B27B] [&_[role=slider]]:border-[#34B27B]"
                  />
                </div>
              )}

              {/* Smoothness Slider (Screen Studio style) */}
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-400">Smoothness (Delay)</span>
                  <span className="text-[10px] text-slate-500 font-mono">{cursorSettings.smoothness ?? 50}%</span>
                </div>
                <Slider
                  value={[cursorSettings.smoothness ?? 50]}
                  onValueChange={([value]) => onCursorSettingsChange({ ...cursorSettings, smoothness: value })}
                  min={0}
                  max={100}
                  step={5}
                  className="[&_[role=slider]]:bg-[#34B27B] [&_[role=slider]]:border-[#34B27B]"
              ...
              {/* Trail Length */}
              {cursorSettings.motionBlur && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-400">Trail Length</span>
                    <span className="text-[10px] text-slate-500 font-mono">{cursorSettings.trailLength ?? 5}</span>
                  </div>
                  <Slider
                    value={[cursorSettings.trailLength ?? 5]}
                    onValueChange={([value]) => onCursorSettingsChange({ ...cursorSettings, trailLength: value })}
                    min={1}
                    max={15}
                    step={1}
                    className="[&_[role=slider]]:bg-[#34B27B] [&_[role=slider]]:border-[#34B27B]"
              ...
              {/* Auto-Hide */}
              <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                <span className="text-[10px] text-slate-300">Auto-Hide when idle</span>
                <Switch
                  checked={cursorSettings.autoHide ?? true}
                  onCheckedChange={(checked) => onCursorSettingsChange({ ...cursorSettings, autoHide: checked })}
                  className="data-[state=checked]:bg-[#34B27B] scale-75"
                />
              </div>
              ...
              {cursorSettings.autoHide && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-400">Hide Delay</span>
                    <span className="text-[10px] text-slate-500 font-mono">{((cursorSettings.autoHideDelay ?? 3000) / 1000).toFixed(1)}s</span>
                  </div>
                  <Slider
                    value={[(cursorSettings.autoHideDelay ?? 3000) / 1000]}
                    onValueChange={([value]) => onCursorSettingsChange({ ...cursorSettings, autoHideDelay: value * 1000 })}
                    min={0.5}
                    max={10}
                    step={0.5}
                    className="[&_[role=slider]]:bg-[#34B27B] [&_[role=slider]]:border-[#34B27B]"
              ...
              {/* Cursor Style */}
              <div className="space-y-1">
                <div className="text-[10px] text-slate-400">Cursor Style</div>
                <div className="flex gap-1">
                  {(['pointer', 'circle', 'crosshair'] as const).map((style) => (
                    <button
                      key={style}
                      onClick={() => onCursorSettingsChange({ ...cursorSettings, cursorStyle: style })}
                      className={cn(
                        "flex-1 py-1.5 text-[10px] font-medium rounded transition-all capitalize",
                        (cursorSettings.cursorStyle ?? 'pointer') === style
                          ? "bg-[#34B27B] text-white"
                          : "bg-white/5 text-slate-400 hover:bg-white/10"
                      )}
                    >
                      {style === 'pointer' ? 'Arrow' : style}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {/* Corner Style Toggle */}
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
            <div className="text-xs font-medium text-slate-200">Squircle</div>
            <Switch
              checked={cornerSettings?.style === 'squircle'}
              onCheckedChange={(checked) => {
                if (onCornerSettingsChange && cornerSettings) {
                  onCornerSettingsChange({ ...cornerSettings, style: checked ? 'squircle' : 'rounded' });
                }
              }}
              className="data-[state=checked]:bg-[#34B27B]"
            />
          ...
          {cornerSettings && onCornerSettingsChange && (cornerSettings.radius ?? cornerRadius) > 0 && (
            <div className="col-span-2 p-3 rounded-xl bg-white/5 border border-white/5">
              <div className="text-xs font-medium text-slate-200 mb-3">Individual Corners</div>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => onCornerSettingsChange({ ...cornerSettings, topLeft: !cornerSettings.topLeft })}
                  className={cn(
                    "p-2 rounded-lg text-[10px] font-medium transition-all",
                    cornerSettings.topLeft 
                      ? "bg-[#34B27B] text-white" 
                      : "bg-white/5 text-slate-400 hover:bg-white/10"
                  )}
                >
                ...
                <button
                  onClick={() => onCornerSettingsChange({ ...cornerSettings, topRight: !cornerSettings.topRight })}
                  className={cn(
                    "p-2 rounded-lg text-[10px] font-medium transition-all",
                    cornerSettings.topRight 
                      ? "bg-[#34B27B] text-white" 
                      : "bg-white/5 text-slate-400 hover:bg-white/10"
                  )}
                >
                ...
                <button
                  onClick={() => onCornerSettingsChange({ ...cornerSettings, bottomLeft: !cornerSettings.bottomLeft })}
                  className={cn(
                    "p-2 rounded-lg text-[10px] font-medium transition-all",
                    cornerSettings.bottomLeft 
                      ? "bg-[#34B27B] text-white" 
                      : "bg-white/5 text-slate-400 hover:bg-white/10"
                  )}
                >
                ...
                <button
                  onClick={() => onCornerSettingsChange({ ...cornerSettings, bottomRight: !cornerSettings.bottomRight })}
                  className={cn(
                    "p-2 rounded-lg text-[10px] font-medium transition-all",
                    cornerSettings.bottomRight 
                      ? "bg-[#34B27B] text-white" 
                      : "bg-white/5 text-slate-400 hover:bg-white/10"
                  )}
                >
